...
class Dataset(data.Dataset):
    def __init__(self, args, is_normal=True, transform=None, test_mode=False):
        ...
        if self.dataset == 'shanghai':
            if test_mode:
                self.rgb_list_file = ROOT/'list/shanghai-i3d-test-10crop.list'
            else:
                self.rgb_list_file = ROOT/'list/shanghai-i3d-train-10crop.list'
                
         ## ADDED   
        elif self.dataset =='ZC':
            if test_mode:
                self.rgb_list_file = ROOT/'list/ZC-i3d-test-10crop.list'
            else:
                self.rgb_list_file = ROOT/'list/ZC-i3d-train-10crop.list'
        else:
            if test_mode:
    ...
    def _parse_list(self):
        ...
        if self.test_mode is False:
            if self.dataset == 'shanghai':
                ...
                if self.is_normal:
                    self.list = self.list[63:]
                    print('normal list for ZC')
                    print(self.list)
                else:
                    self.list = self.list[0]
                    print('abnormal list for ZC')
                    print(self.list)
